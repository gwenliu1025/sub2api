...
require_exact_line "$version_file" "0.2.5"
require_exact_line "deploy/.env.example" "SUB2API_IMAGE=ghcr.io/gwenliu1025/sub2api:0.2.5"
...
require_contains "deploy/docker-deploy.sh" 'https://raw.githubusercontent.com/gwenliu1025/sub2api/v0.2.5/deploy'
require_absent "deploy/docker-deploy.sh" 'Wei-Shaw/sub2api'
require_exact_line "deploy/.env.example" 'APPLE_CONTAINER_SUB2API_IMAGE=ghcr.io/gwenliu1025/sub2api:0.2.5'
require_contains "deploy/apple-container.sh" 'ghcr.io/gwenliu1025/sub2api:0.2.5'
require_absent "deploy/apple-container.sh" 'weishaw/sub2api:latest'

require_exact_line "deploy/docker-deploy.sh" 'GITHUB_RAW_URL="https://raw.githubusercontent.com/gwenliu1025/sub2api/v0.2.5/deploy"'
for release_doc in deploy/README.md deploy/DOCKER.md deploy/APPLE_CONTAINER.md; do
  ...
  require_contains "$release_doc" 'ghcr.io/gwenliu1025/sub2api:0.2.5'
done
...
for compose in $compose_files; do
  require_contains "$compose" 'image: ${SUB2API_IMAGE:-ghcr.io/gwenliu1025/sub2api:0.2.5}'
