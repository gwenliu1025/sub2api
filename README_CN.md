...
- **Kiro-Go 等效缓存 V2**：通过精确分配器重写 Anthropic 缓存用量展示，同时保持
  上游原始 token 总量不变；流式请求使用 Kiro-Go 最终用量帧执行分配。
...
重建后的 `v0.1.152` 沿用已经合并的原作者 `v0.1.152` 基线，包括 xAI API Key 账号、
Grok OAuth 提示词缓存、
Codex alpha/search 转发与按次计费、更完整的工具桥接，以及 Codex 身份、
remote compaction、compact 心跳和 Anthropic 缓存创建用量修复。本 fork 同时加入
Kiro-Go 等效缓存 V2、原始成本锁定、等效缓存审计持久化、fork 自有 Release 与
Docker 更新元数据，并修复流式请求过早使用初始估算用量的问题，使流式与同步
请求都使用最终完整用量执行相同的 V2 分配。本次同版本重建将同一会话再次创建
缓存前的最低读取次数从 4 次调整为 3 次，并将约 10% 已通过保护条件的读取候选
稳定提升为创建型，适度提高缓存创建事件出现频率。
...
1. 点击“准备镜像”：从 `gwenliu1025/sub2api` 读取 `vX.Y.Z` Release，拉取并校验对应的 `ghcr.io/gwenliu1025/sub2api:X.Y.Z` 镜像，服务不会中断。
2. 点击“立即重启”：仅重建 `sub2api` 切换到已准备镜像，等待健康检查；失败时自动恢复到上一镜像。

只有 GitHub Release 和同版本 GHCR 镜像同时存在时，管理面板才会提供该 Docker 更新。
