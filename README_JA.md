...
Docker デプロイは、次の 2 段階で更新します。

1. 「Prepare Image」をクリックすると、`gwenliu1025/sub2api` の `vX.Y.Z` Release を読み取り、対応する `ghcr.io/gwenliu1025/sub2api:X.Y.Z` イメージを取得・検証します。サービスは停止しません。
2. 「Restart To Switch」をクリックすると、準備済みイメージへ切り替えるために `sub2api` だけを再作成し、ヘルスチェックを待機します。失敗時は直前のイメージへ自動復元します。

管理パネルで Docker 更新を行うには、GitHub Release と同じバージョンの GHCR イメージの両方が必要です。
